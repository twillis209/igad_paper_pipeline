...
dat <- fread(snakemake@input[[1]], select = c('abbrv', 'pretty_name', 'N0', 'N1', 'Ancestry', 'First_Author', 'citation', 'URL'))

dat <- dat[abbrv %in% snakemake@params$abbrvs]

dat[!is.null(N1), Neff := round(2/(1/N0 + 1/N1))]
dat[N1 == 0, Neff := N0]
...
fwrite(dat[order(Ancestry), .(Phenotype = pretty_name, Cases = N1, Controls = N0, `Effective sample size` = Neff, Ancestry, Citation = First_Author, URL)], file = snakemake@output[['tsv']], sep = '\t')

dat[First_Author %in% c('FinnGen', 'Pan-UKB', 'IMSGC', 'UKB'), citation := paste(First_Author, citation, sep = ' ')]

dat[order(Ancestry), .(pretty_name, N1, N0, Neff, Ancestry, citation)] %>%
  kbl(
    format = 'latex',
