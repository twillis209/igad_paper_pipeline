...
manhplot <- ggplot(gwas_data, aes(x = bp_cum, y = p, color = as_factor(chr))) +
  ...
  scale_y_neglog10(limits = c(1, 1e-65))+
  labs(x = NULL, 
       y = "-log<sub>10</sub>(p)") + 
  theme( 
...
if(!is.null(snakemake@input$annotations)) {
  ...
  manhplot <- manhplot+
    ...
    new_scale_color()+
    geom_label_repel(aes(colour = text_colour, label = Gene), size = 3, data = merged_dat[Gene == 'RUNX3'], min.segment.length = 0, nudge_y = -10, angle = 90)+
    geom_label_repel(aes(colour = text_colour, label = Gene), size = 3, data = merged_dat[Gene != 'RUNX3'], min.segment.length = 0, nudge_y = 35, angle = 90)+
